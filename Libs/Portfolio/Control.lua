...
function Portfolio.Control.GetValueText(control, key)
	...
	if (type(text) == "function") then
		success, result = pcall( text, value )
	elseif (type(text) == "string") then
		if value ~= nil and ( strfind(text, "%%s") or strfind(text, "%%d") ) then
			success, result = pcall( format, text, value )
		else
			success = true
			result = text
		end
...
function Portfolio.Control.SetRelativePoint(self, point, anchor, relativePoint, offsetX, offsetY)
	if (not anchor and not self.prev) then
		-- Title, first option, anchor to the parent optionsFrame.scrollChild
		self:SetPoint("TOPLEFT", 12, -12)
	else
		...
		if not offsetY then
			offsetY = -8
			if self.type == CONTROLTYPE_HEADER then
				offsetY = -24
			end
			if anchor.type == CONTROLTYPE_SLIDER then
				offsetY = offsetY - 4
			end
			...
			if self.type == CONTROLTYPE_EDITBOX then
				offsetY = offsetY - 12
			end
		end
		...
		if not offsetX then
			offsetX = 0
			if anchor.type == CONTROLTYPE_COLORPICKER then
				offsetX = offsetX - 5
			end
			if self.type == CONTROLTYPE_COLORPICKER then
				offsetX = offsetX + 5
			end
			if anchor.type == CONTROLTYPE_EDITBOX then
				offsetX = offsetX - 2
			end
			if self.type == CONTROLTYPE_EDITBOX then
				offsetX = offsetX + 2
...
--[[-- Update the control width to adjust text wrapping.
	Only needs to be called if the position of the control is moved.
	(headers and texts)
	
	@name			UpdateTextWrap
	@param self		header or text control
	@usage			control:UpdateTextWrap()
--]]--
function Portfolio.Control.UpdateTextWrap(self)
