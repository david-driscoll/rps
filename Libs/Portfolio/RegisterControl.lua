...
--[[
	@class table
	@name Control Types
	@field CONTROLTYPE_HEADER		Large text with optional sub text
	@field CONTROLTYPE_TEXT			Text block
	@field CONTROLTYPE_CHECKBOX		Checkbox with text on the right
	@field CONTROLTYPE_DROPDOWN		DropDown menu with text on the control and header text above
	@field CONTROLTYPE_SLIDER		Slider with a numerical range, min/max text, and text above
	@field CONTROLTYPE_BUTTON		Button with text inside
	@field CONTROLTYPE_INPUT		(NYI) Text input field
	@field CONTROLTYPE_COLORPICKER	(NYI) Color square, opens color wheel
	@field CONTROLTYPE_CUSTOM		(NYI) Custom frame
]]--
...
--[[-- Control population called on all controls.
	...
	@param control		(frame) 
	@usage				Portfolio.PopulateValueControl(optionsFrame, option , optionType, control)
--]]--
